...
import { newProductId, resizeImage, saveProduct, type ProductRow } from '../lib/products'
...
/** Ajout / modification d'un article par l'admin : categorie, fiche, variantes et prix, photos. */
export function ArticleFormModal({ initial, defaultCategoryId, onClose }: { initial?: ProductRow; defaultCategoryId?: string | null; onClose: () => void }) {
  ...
  const [rows, setRows] = useState<VariantRow[]>(toRows(initial?.variants ?? []))
  ...
  return (
    <Modal title={initial ? "Modifier l'article" : 'Ajouter un article'} onClose={onClose} wide>
      <div className="space-y-4">
        ...
        <div>
          <div className="mb-1 text-xs font-semibold text-slate-600">Variantes et prix TTC (DT)</div>
          <div className="space-y-2">
            {rows.map((r, i) => (
              <div key={i} className="flex items-center gap-2">
                <input className={inputCls} value={r.label} onChange={e => patch(i, { label: e.target.value })} placeholder="Variante (ex : 10 cm, Noir...)" />
          ...
          <button onClick={() => setRows(rs => [...rs, { label: '', price: '', code: '' }])} className="mt-2 flex items-center gap-1.5 text-xs font-bold text-teal-dark"><Plus size={14} /> Ajouter une variante</button>
        ...
        <p className="rounded-lg bg-teal/10 px-3 py-2 text-xs text-slate-600">Cet article sera visible immediatement dans votre application et chez les commerciaux, <b>pas sur le site officiel</b>.</p>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={save} disabled={busy} className="rounded-xl bg-navy px-6 py-2 text-sm font-bold text-white disabled:opacity-60">{busy ? 'Enregistrement...' : 'Enregistrer et publier'}</button>
        </div>
      </div>
    </Modal>
  )
